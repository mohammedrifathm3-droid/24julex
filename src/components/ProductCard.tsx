  ...
  return (
    ...
    >
      <CardContent className="p-0">
        {/* Product Image */}
        <Link href={`/product/${product.id}`} className="block relative aspect-square overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
          <Image
            src={mainImage}
            alt={product.name}
            fill
          ...
          <div className={`absolute inset-0 bg-black/50 flex items-center justify-center space-x-2 transition-opacity duration-300 z-10 ${isHovered ? 'opacity-100' : 'opacity-0'
            }`}>
            <Button
              size="sm"
              className="bg-yellow-400 hover:bg-yellow-500 text-black font-medium disabled:opacity-70"
